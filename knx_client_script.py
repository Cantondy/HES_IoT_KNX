...
def send_knx_request(
    ...
    logging.debug('>>> CONNECTION_REQUEST:\n{}'.format(conn_req))
    logging.info('>>> CONNECTION_REQUEST')
    sock.sendto(conn_req.frame, (gateway_ip, gateway_port))

    # <- Connection response
    logging.debug('waiting for a connection response (CONNECTION_RESPONSE -> conn_resp)')
    data_recv, addr = sock.recvfrom(buf_size)
    conn_resp = knxnet.decode_frame(data_recv)
    logging.info('Connection response status (KNX): {}'.format(conn_resp.status))
    logging.info('Connection response data endpoint (KNX): {}'.format(conn_resp.data_endpoint))
    ...
    """Check if error occured in response and disconnect
    request            : Request with status code to check
    disconnect_request : Disconnect request
    """
    def checkError(request, disconnect_request):
        if request.status != 0:
            logging.debug('>>> DISCONNECT_REQUEST:\n{}'.format(disconnect_request))
            logging.info('>>> DISCONNECT_REQUEST')
            sock.sendto(disconnect_request.frame, (gateway_ip, gateway_port))

            # <- Disconnect response
            data_recv, addr = sock.recvfrom(buf_size)
            disc_resp = knxnet.decode_frame(data_recv)
            logging.info('Disconnect reponse channel ID (KNX): {}'.format(disc_resp.channel_id))
            logging.info('Disconnect reponse status (KNX): {}'.format(disc_resp.status))

            if disc_resp.status != 0:
                logging.error('ERROR : Disconnect Request')
            else:
                logging.info('SUCCESS : Disconnect OK')
            
            print("Script terminated suddenly : Connection Error")
    ...
    checkError(conn_resp, disc_req)

    ########################################################################
    ### If OK, make a CONNECTION_STATE_REQUEST and CONNECTION_STATE_RESPONSE
    ########################################################################
    ...
    logging.debug('>>> CONNECTION_STATE_REQUEST:\n{}'.format(conn_state_req))
    logging.info('>>> CONNECTION_STATE_REQUEST')
    sock.sendto(conn_state_req.frame, (gateway_ip, gateway_port))

    # <- Connection state response
    data_recv, addr = sock.recvfrom(buf_size)
    conn_state_resp = knxnet.decode_frame(data_recv)
    logging.info('Connection state reponse channel ID (KNX): {}'.format(conn_state_resp.channel_id))
    logging.info('Connection state reponse status (KNX): {}'.format(conn_state_resp.status))
    ...
    BOOL_READ_REQUEST = False

    if tunn_req.apci == 0:
        BOOL_READ_REQUEST = True

    logging.debug('>>> TUNNELING_REQUEST:\n{}'.format(tunn_req))
    logging.info('>>> TUNNELING_REQUEST')
    sock.sendto(tunn_req.frame, (gateway_ip, gateway_port))

    # <- Tunneling ACK
    data_recv, addr = sock.recvfrom(buf_size)
    tunnel_ack = knxnet.decode_frame(data_recv)
    logging.info('Tunneling ACK channel ID (KNX): {}'.format(tunnel_ack.channel_id))
    logging.info('Tunneling ACK status (KNX): {}'.format(tunnel_ack.status))

    ### Check if status from connection_state_response is OK, else disconnect
    checkError(tunnel_ack, disc_req)

    # <- Tunneling REQUEST
    data_recv, addr = sock.recvfrom(buf_size)
    tunnel_req_knx = knxnet.decode_frame(data_recv)
    logging.info('Tunneling request data (KNX): {}'.format(tunnel_req_knx.data))
    logging.info('Tunneling request apci (KNX): {}'.format(tunnel_req_knx.apci))
    logging.info('Tunneling request data_service (KNX): {}'.format(tunnel_req_knx.data_service))

    ack_status = 0

    # -> Tunneling ACK
    tunn_ack_client = knxnet.create_frame(
        knxnet.ServiceTypeDescriptor.TUNNELLING_ACK,
        channel_id,
        ack_status
    )

    logging.debug('>>> TUNNELING_ACK:\n{}'.format(tunn_ack_client))
    logging.info('>>> TUNNELING_ACK')
    sock.sendto(tunn_ack_client.frame, (gateway_ip, gateway_port))
    
    ########################################################################
    ### Get a TUNNELING_REQUEST from KNX if this is a Read commande
    ########################################################################

    # <- Tunneling REQUEST if read commande
    if BOOL_READ_REQUEST == True:
        data_recv, addr = sock.recvfrom(buf_size)
        tunnel_req_knx_read = knxnet.decode_frame(data_recv)
        logging.info('Tunneling request data (KNX-READ): {}'.format(tunnel_req_knx_read.data))
        logging.info('Tunneling request apci (KNX-READ): {}'.format(tunnel_req_knx_read.apci))
        logging.info('Tunneling request data_service (KNX-READ): {}'.format(tunnel_req_knx_read.data_service))

        print('KNX response for Read commande : {}'.format(tunnel_req_knx_read.data))

    # -> Final disconnect request and response
    logging.debug('>>> DISCONNECT_REQUEST:\n{}'.format(disc_req))
    logging.info('>>> DISCONNECT_REQUEST')
